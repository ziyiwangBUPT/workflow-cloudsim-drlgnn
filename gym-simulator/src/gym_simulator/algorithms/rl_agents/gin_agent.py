...
class BaseGinNetwork(nn.Module):
    ...
    def forward(
        ...
        job_nodes = torch.arange(self.n_jobs).unsqueeze(1).expand(self.n_jobs, self.n_machines)
        ...
        x_job_job_edges = torch.zeros(job_job_edge_count, self.embedding_dim)  # Placeholders for job-job edges
...
class GinActor(nn.Module):
    ...
    def forward(
        ...
        action_scores = torch.ones_like(job_machine_connectivity, dtype=torch.float32) * -1e8
